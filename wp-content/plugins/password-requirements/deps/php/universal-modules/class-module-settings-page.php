...
class Module_Settings_Page extends Utils\Module {
	 ...
	 * @var string
	 */
	const OPTION_KEY__SHOULD_INITIATE_ONBOARDING = 'password_requirements__should_initiate_onboarding';
	...
	public function setup_class_properties(): void {
		...
		if ( null === $this->page_title ) {
			$this->page_title = __( 'Settings', 'password-requirements' );
		}
			...
			$actions = array_merge(
				[
					'settings' => sprintf(
						...
						__( 'Settings', 'password-requirements' ),
					),
				],
				$actions,
			);
	...
	public function enqueue_scripts(): void {
		...
		$asset->enqueue_style(
			array_merge(
				 ...
				 */
				apply_filters( 'password_requirements__settings_page_stylesheet_additional_dependencies', [] ),
			),
		);
		...
		$asset->enqueue_script(
			 ...
			 */
			apply_filters(
				'password_requirements__settings_page_script_inline_data',
				[
					'nonce'     => $nonce->create(),
			 ...
			 */
			apply_filters( 'password_requirements__settings_page_script_additional_dependencies', [] ),
		);
		 ...
		 */
		do_action( 'password_requirements__enqueue_settings_page_scripts' );
	}
