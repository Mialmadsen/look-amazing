 ...
 */
export const FieldsGroup = ( { children, className = '', help = '', label = '', withBaseControl = false, withBoxBorder = false, withReducedGap = false, ...otherProps } ) => {
	let classNames = [
		className,
		'tsc-fields-group',
	...
	if ( withBaseControl ) {
		...
		const { baseControlProps, controlProps } = useBaseControlProps( { preferredId: buildId( 'npm-components', 'fields-group', cleanForSlug( label ) ) } );
