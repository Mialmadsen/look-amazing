...
class Settings {
	...
	protected function load_from_db(): array {
		 ...
		 *
		 * @param array<string,mixed> $defaults Empty array if no default settings defined, array of default settings otherwise.
		 */
		$defaults = apply_filters( 'password_requirements__settings_default', $this->get_default() );

		/** @var false|array<string,mixed> $data */
		$data = $this->container->is_network_enabled()
			? get_network_option( get_current_network_id(), $this->get_option_key(), $defaults )
			: get_option( $this->get_option_key(), $defaults );
		 ...
		 */
		return apply_filters( 'password_requirements__settings_loaded', $data );
	}
